 ...
 *   Gate: BILLKO_LAKE_ENABLED=true + BILLKO_LAKE_API_KEY or username/password.
 *   Does not move SEPA money — reports tax-side payment on outgoing eInvoices.
 */
function remainingWiringNotes() {
  return {
    ...
    billkoLake:
      "Imperial Lake: /admin/payouts/billko/* — incoming/outgoing list + ereporting/paid. Gate: BILLKO_LAKE_ENABLED + lake credentials. After live Fiscalize, best-effort payment report (ClearingBetweenPartners).",
    organizerGates: {
      ...
      lakeDelivered: [
        "Imperial Lake client + admin billko routes",
        "report-payment after Fiscalize when lake ready",
      ],
      stillManual: ["bank upload UI (manual — never auto-submit)"],
      stillOpen: [
        "UBL document/send for native Lake commission invoices (api-client invoices may not appear in Lake)",
        "Buyer-side payment record for incoming invoices (not in Imperial Lake public API)",
      ],
    },
    webhookFilters:
