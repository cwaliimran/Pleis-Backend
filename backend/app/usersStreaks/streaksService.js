...
const Streaks = require("@UsersStreaksModel"); // Assuming the model is in models directory

const createStreak = async ({ user, companyOrganizer, visits = 0, points = 0 }) => {
  return await streakRepo.createStreak({ user, companyOrganizer, visits, points });
};
...
const getStreaks = async ({ companyOrganizer, page, limit, keyword, status, date, orderSort = "asc" }) => {
  ...
  // Populate user and companyOrganizer references
  streaks = await Streaks.populate(streaks, [{ path: 'user' }, { path: 'companyOrganizer' }]);
...
const updateStreak = async (id, data) => {
  ...
  if (Object.keys(updateData).length === 0) {
    const streak = await streakRepo.findStreakById(id).populate('user companyOrganizer');
    return streak;
  }

  const updated = await streakRepo.findByIdAndUpdate(id, updateData, { new: true }).populate('user companyOrganizer');
  return updated;
};
