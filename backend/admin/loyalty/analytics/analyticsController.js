...
const getAnalytics = async (req, res) => {
  ...
  if(!user||!companyOrganizer){
    return sendResponse({
      res,
      statusCode: 400,
      translationKey: "user_and_companyOrganizer_required",
    });
  }
  ...
  const getTransactions = async (req, res) => {
    let { page=1, limit=5, companyOrganizer,organizations, user } = req.query;
    let { timezone } = req.user || "UTC";
    if (req.user.userType === "organizer") {
      companyOrganizer = req.user._id;
      if(organizations){
  ...
  limit = parseInt(limit);
    try {
      const {data, meta} = await AnalyticsService.getTransactions({
        page,
        limit,
        timezone,
        companyOrganizer,
...
const getAnalyticsStats = async (req, res) => {
  ...
  const getsummary = async (req, res) => {
    ...
    if(!companyOrganizer&&!user){
      return sendResponse({
        res,
        statusCode: 400,
        translationKey: "invalid_request",
        error: "Either companyOrganizer or user must be provided",
      });
    }
    limit = parseInt(limit);
    try {
      ...
      return sendResponse({
        res,
        statusCode: 200,
        translationKey: "Analytics_summary_fetched_successfully",
        data
      });
    } catch (error) {
      return sendResponse({
