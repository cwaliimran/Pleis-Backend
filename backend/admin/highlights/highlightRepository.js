...
const getHighlightsWithFilters = async (query, keyword, skip, limit, sortBy, sortOrder) => {
  let cacheKey = buildHighlightsCacheKey({
    scope: "admin",
    skip,
    limit,
  });
      ...
      if (sortBy && sortOrder) {
        ...
        if (sortBy === "title") {
          ...
          pipeline.push({
            $addFields: {
              typeSort: {
                $cond: [{ $eq: ["$type", "organization"] }, 0, 1]
              },
              organizationNameSort: {
                $toLower: { $ifNull: ["$object.basicInfo.name", ""] }
              }
            }
          });
          ...
          pipeline.push({
            $addFields: {
              typeSort: {
                $cond: [{ $eq: ["$type", "event"] }, 0, 1]
              },
              eventNameSort: {
                $toLower: { $ifNull: ["$object.basicInfo.title", ""] }
              }
            }
          });
