...
const getUpdatess = async ({ timezone, page, limit, keyword, status, userId, date, range, sortBy, sortOrder }) => {
  const skip = limit === 0 ? 0 : (page - 1) * limit;
  const today = getCurrentDateInTimezone({ timezone, isDateOnly: true });
  let { updates, meta } = await UpdatesRepo.getUpdatess({ timezone, page, limit, keyword, status, userId, date, range, today, skip, sortBy, sortOrder });
...
const getUpdatesByEventIdService = async (eventId) => {
  let updates = await UpdatesRepo.getUpdatesByEventId(eventId);
  //format updates
