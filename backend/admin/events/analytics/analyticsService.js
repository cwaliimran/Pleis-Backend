...
const { getUserIdsForOrganization } = require("@appEngagement/engagementEventsRepository");
...
const { buildViewOverTime } = require("./utils/buildViewOverTime");
...
const getAnalytics = async ({ dateFilter, timezone, companyOrganizer, event }) => {
  ...
  const promises = [
    ...
    getViews(users, event, dateFilter, timezone), // used
    getRawInterestDataByOrganizer(users),
    getEventViewsByTimeRaw(users, event, dateFilter, timezone),
...
const getByTimeRaw = async (users, event, dateFilter, timezone) => {
  const rows = await AnalyticsRepo.getByTimeRaw(users, event, dateFilter, timezone);
  return buildByOverTime(rows);
};
const getEventByTimeRaw = async (users, event, dateFilter, timezone) => {
  const rows = await AnalyticsRepo.getEventByTimeRaw(users, event, dateFilter, timezone);
...
const getViews = async (users, event, dateFilter, timezone) => {
  const rows = await AnalyticsRepo.getViews(users, event, dateFilter, timezone);
