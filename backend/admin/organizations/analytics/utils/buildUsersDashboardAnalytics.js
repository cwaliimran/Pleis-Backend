...
const buildUserDashboardAnalytics = (users = []) => {
    ...
    if (u.dob && typeof u.dob === "string") {
      ...
      if (!isNaN(dob)) {
        const age = Math.floor(
          (now - dob) / (365.25 * 24 * 60 * 60 * 1000) // Calculate age in years
        );

        if (age >= 18 && age < 25) ageBuckets["18-25"][gender]++;
        else if (age < 35) ageBuckets["25-34"][gender]++;
        else if (age < 45) ageBuckets["35-44"][gender]++;
        else if (age < 55) ageBuckets["45-54"][gender]++;
        else if (age >= 55) ageBuckets["55+"][gender]++;
      }
    }
  ...
  return {
    /* -------- Age Demographics -------- */
    ageDemographics: Object.entries(ageBuckets).map(([ageGroup, genderStats]) => ({
      ageGroup,
