...
const getDashboard = async ({ dateFilter, timezone, companyOrganizer,CompanyOrganizations }) => {
  ...
  const promises = [
    ...
    dashboardRepo.getEventStats({ dateFilter, timezone, companyOrganizer,CompanyOrganizations }),
    dashboardRepo.getTicketsSoldStats({ dateFilter, timezone, companyOrganizer,CompanyOrganizations }),
    dashboardRepo.getAverageTicketPriceStats({ dateFilter, timezone, companyOrganizer,CompanyOrganizations }),
    dashboardRepo.getAverageRevenuePerUserStats({ dateFilter, timezone, companyOrganizer,CompanyOrganizations }),
    dashboardRepo.getTotalRevenueStats({ dateFilter, timezone, companyOrganizer,CompanyOrganizations }),
    getOrganizerPerformanceComparisonService({ timezone,companyOrganizer, CompanyOrganizations }),
    ...
    getEventsOverTimeService(companyOrganizer,CompanyOrganizations),
    getTrends(companyOrganizer,CompanyOrganizations),
    dashboardRepo.getTotalMobilePaymentsStats({ dateFilter, timezone }),
  ];
  if (companyOrganizer) {
    promises.push(
      dashboardRepo.getOrganizationsStats({ dateFilter, timezone, companyOrganizer,CompanyOrganizations })
    );
  }
  if (companyOrganizer) {
    promises.push(
  ...
  if (companyOrganizer) {
    promises.push(
      dashboardRepo.getReservationsStats({ dateFilter, timezone, companyOrganizer,CompanyOrganizations })
    );
  }
  if (companyOrganizer) {
    promises.push(
  ...
  if (companyOrganizer) {
    promises.push(
      getTopViewedEvents(companyOrganizer,CompanyOrganizations)
    );
  }
  if (companyOrganizer) {
    promises.push(
  ...
  if (companyOrganizer) {
    promises.push(
      getRawTopPerformingEvents(companyOrganizer,CompanyOrganizations)
    );
  }
...
const getTrends = async (companyOrganizer, CompanyOrganizations) => {
  const [salesRows, revenueRows] = await Promise.all([
    dashboardRepo.getTotalTrendSales(companyOrganizer,CompanyOrganizations),
    dashboardRepo.getTotalTrendRevenue(companyOrganizer,CompanyOrganizations)
  ]);
...
const getEventsOverTimeService = async (companyOrganizer, CompanyOrganizations) => {
  let rows;
  if (!companyOrganizer) {
    rows = await dashboardRepo.getEventsOverTimeRaw();
  }
  if (companyOrganizer) {
    rows = await dashboardRepo.getEventsViewsOverTimeService(companyOrganizer, CompanyOrganizations);
  }
...
const getTopViewedEvents = async (companyOrganizer, CompanyOrganizations) => {
  let rows;
  if (companyOrganizer) {
    rows = await dashboardRepo.getTopViewedEvents(companyOrganizer, CompanyOrganizations);
  }
...
const getRawTopPerformingEvents = async (companyOrganizer, CompanyOrganizations) => {
  const rows = await dashboardRepo.getRawTopPerformingEvents(companyOrganizer, CompanyOrganizations);
