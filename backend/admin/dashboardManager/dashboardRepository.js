...
// ---------------- USERS ----------------
const getUserStats = async ({ dateFilter, timezone, companyOrganizer, CompanyOrganizations }) => {
  const ranges = getDateRanges({ dateFilter, timezone });
  ...
  if (companyOrganizer && CompanyOrganizations && CompanyOrganizations.length > 0) {
    ...
    const baseMatch = {
      ...(CompanyOrganizations && CompanyOrganizations.length > 0
        ? {
          _id: {
            $in: CompanyOrganizations.map(
...
// ---------------- EVENTS ----------------
const getEventStats = async ({ dateFilter, timezone, status, companyOrganizer, CompanyOrganizations }) => {
  const ranges = getDateRanges({ dateFilter, timezone });
  if (CompanyOrganizations && CompanyOrganizations.length > 0) {
    ...
    const baseMatch = {
      ...
      ...(CompanyOrganizations && CompanyOrganizations.length > 0 && {
        "basicInfo.organization": { $in: CompanyOrganizations.map(id => new mongoose.Types.ObjectId(id)) },
      })
    };
...
const getTicketsSoldStats = async ({ dateFilter, timezone, companyOrganizer, CompanyOrganizations }) => {
  const ranges = getDateRanges({ dateFilter, timezone });
  if (CompanyOrganizations && CompanyOrganizations.length > 0) {
    ...
    const baseMatch = {
      ...
      ...(CompanyOrganizations && CompanyOrganizations.length > 0 && {
        organization: { $in: CompanyOrganizations.map(id => new mongoose.Types.ObjectId(id)) },
      }),
    };
    ...
    return {
      ticketsSoldCurrent: 0,
      ticketsSoldPrevious: 0,
    };
  };
};
...
const getAverageTicketPriceStats = async ({ dateFilter, timezone, companyOrganizer,CompanyOrganizations }) => {
  const ranges = getDateRanges({ dateFilter, timezone });
...
const getOrganizerPerformanceByMonth = async ({
  ...
  if (CompanyOrganizations && CompanyOrganizations.length > 0) {
    match.organization = { $in: CompanyOrganizations.map(id => new mongoose.Types.ObjectId(id)) };
  }
...
const getAverageRevenuePerUserStats = async ({ dateFilter, timezone, companyOrganizer,CompanyOrganizations }) => {
  const ranges = getDateRanges({ dateFilter, timezone });
    ...
    const result = await TicketingOrders.aggregate([
      {
        $match: {
          purpose: "eventTicketPurchase",
          ...(CompanyOrganizations && { organization: { $in: CompanyOrganizations.map(id => new mongoose.Types.ObjectId(id)) } }),
          ...(range && { createdAt: range }),
        },
      },
      {
...
const getTotalTrendSales = async (companyOrganizer,CompanyOrganizations) => {
...
const getTotalRevenueStats = async ({ dateFilter, timezone, companyOrganizer,CompanyOrganizations }) => {
  const ranges = getDateRanges({ dateFilter, timezone });
    ...
    const result = await UnifiedWalletTransactions.aggregate([
      {
        $match: {
          type: "earn",
          ...(range && { createdAt: range }),
          ...(CompanyOrganizations && CompanyOrganizations.length > 0 && { organization: { $in: CompanyOrganizations.map(id => new mongoose.Types.ObjectId(id)) } }),
        },
      },
      {
        $project: {
...
const getOrganizationsStats = async ({ dateFilter, timezone, companyOrganizer, CompanyOrganizations }) => {
  const ranges = getDateRanges({ dateFilter, timezone });
  if (CompanyOrganizations && CompanyOrganizations.length > 0) {
      ...
      const result = await Organizations.aggregate([
        {
          $match: {
            status: { $ne: "deleted" },
            ...(CompanyOrganizations && CompanyOrganizations.length > 0 && {
              _id: { $in: CompanyOrganizations.map(id => new mongoose.Types.ObjectId(id)) },
            }),
            ...(range && { createdAt: range }),
          },
        },
    ...
    return {
      totalOrganizationsCurrent: 0,
      totalOrganizationsPrevious: 0,
    };
  };
}
const getReservationsStats = async ({ companyOrganizer, dateFilter, timezone, CompanyOrganizations }) => {
  const ranges = getDateRanges({ dateFilter, timezone });
  if (CompanyOrganizations && CompanyOrganizations.length > 0) {
    const baseMatch = {
      ...(CompanyOrganizations && CompanyOrganizations.length > 0 && {
        organizationId: { $in: CompanyOrganizations.map(id => new mongoose.Types.ObjectId(id)) },
      }),
    };
    ...
    return {
      totalReservationsCurrent: 0,
      totalReservationsPrevious: 0,
      activeReservationsCurrent: 0,
      activeReservationsPrevious: 0,
      inactiveReservationsCurrent: 0,
      inactiveReservationsPrevious: 0,
      deletedReservationsCurrent: 0,
      deletedReservationsPrevious: 0,
    };
  };
}
...
const getEventsViewsOverTimeService = async (companyOrganizer, CompanyOrganizations) => {
  ...
  return EngagementEvents.aggregate([
    ...
    {
      $match: {
        "event.basicInfo.organization": CompanyOrganizations && CompanyOrganizations.length > 0 ? { $in: CompanyOrganizations.map(id => new mongoose.Types.ObjectId(id)) } : { $exists: true },
...
const getTopViewedEvents = async (companyOrganizer, CompanyOrganizations) => {
  const year = new Date().getFullYear();
...
const getRawTopPerformingEvents = async (companyOrganizer, CompanyOrganizations) => {
  return TicketingOrders.aggregate([
    ...
    -------------------------------- */
    {
      $match: {
        purpose: "eventTicketPurchase",
        organization: CompanyOrganizations && CompanyOrganizations.length > 0 ? { $in: CompanyOrganizations.map(id => new mongoose.Types.ObjectId(id)) } : { $exists: true },
      }
    },
