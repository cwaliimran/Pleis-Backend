...
const { convertUtcToTimezoneAMPM, convertUtcToTimezone } = require("@utils/responseUtil");
...
const userReservationsFormatter = (item, timezone) => {
  ...
  if (cat.timingSlots && cat.timingSlots.dateTimeSlots) {
    ...
    if (Array.isArray(dateTimeSlot) && dateTimeSlot.length === 1) {
      ...
      cat.timingSlots.date = convertUtcToTimezone(slot.date, timezone, "YYYY-MM-DD" );
...
module.exports = { reservationsFormatterAdjustDates, userReservationsFormatter };
