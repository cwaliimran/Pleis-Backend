...
const getReviews = async (data) => {
  try {
    ...
    const pipeline = [
      { $match: { organization: { $in: organizationObjectIds }, status: "active" } },
      ...
      // Lookup user
      {
        $lookup: {
          ...
          localField: "user",
          foreignField: "_id",
          pipeline: [{ $project: { firstName: 1, lastName: 1, profileIcon: 1, location: 1 } }],
          as: "userDetails"
        }
      },
      ...
      ...(data.keyword ? [{
        $match: {
          $or: [
            { comment: { $regex: data.keyword, $options: "i" } },
            { "userDetails.firstName": { $regex: data.keyword, $options: "i" } },
            { "userDetails.lastName": { $regex: data.keyword, $options: "i" } },
            { "userDetails.location.fullAddress": { $regex: data.keyword, $options: "i" } },
            { "organizationDetails.basicInfo.name": { $regex: data.keyword, $options: "i" } }
          ]
        }
      }] : [])
    ];
    ...
    if (sortBy === "userName") {
      pipeline.push({
        $addFields: {
          userNameSort: {
            $toLower: { $concat: [{ $ifNull: ["$userDetails.firstName", ""] }, " ", { $ifNull: ["$userDetails.lastName", ""] }] }
          }
        }
      });
      pipeline.push({ $sort: { userNameSort: sortOrder, _id: -1 } });
    } else {
      pipeline.push({ $sort: { createdAt: sortOrder, _id: -1 } });
    }
    ...
    return {
      data1: reviews.map(r => formatReviewData(r)),
      meta: {
        totalCount: reviews.length // You can calculate total separately if needed
      }
    };
