...
const getGiveaway = async ({ timezone, page, limit, keyword, status, userId, date, range, today, skip }) => {
  ...
  const pipeline = [
    {
      $match: {
        ...(userId && { creator: new mongoose.Types.ObjectId(userId) })  // Match creator if userId is provided
      }
    },
    ...
    // Step 4: Keyword search for event and ticket titles
    ...(keyword ? [{
      $match: {
        $or: [
          { title: { $regex: keyword, $options: "i" } }, // Search in 'title' of the giveaway
          { "event.basicInfo.title": { $regex: keyword, $options: "i" } }, // Search in 'title' of the event
          { "ticket.title": { $regex: keyword, $options: "i" } }, // Search in 'title' of the ticket
        ],
      },
    }] : []),
    ...
    // Step 7: Project only the title from the ticket and event
    {
      $project: {
        eventTitle: "$event.basicInfo.title", // Only return event title
        ticketType: "$ticket.title", // Only return ticket title
        title: 1, 
        numberOfWinners: 1,
        ticketsPerWinner: 1,
        // startDateTime: 1,
