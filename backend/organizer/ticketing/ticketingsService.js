...
const getOrganizationTicketings = async ({
  timezone,
  page,
  limit,
  keyword,
  ...
  // 🔹 Get events for ALL organizations
  const organizationEvents = organizationIds.length
    ? await getEventIdsByOrganization(organizationIds)
    : [];
  ...
  // 🔹 Apply event filter only if events exist
  if (eventIds.length) {
  ...
  return {
    ticketings: formattedTicketings,
    meta
  };
};
