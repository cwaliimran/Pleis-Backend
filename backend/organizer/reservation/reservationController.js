...
const reservationService = require("../../admin/reservation/reservationService");
...
const getUserReservations = async (req, res) => {
  const { page, limit } = parsePaginationParams(req);
  const { keyword, status = "confirmed", date, range, organizationsId, companyOrganizer, reservationId } = req.query;
...
const getReservations = async (req, res) => {
  const { page, limit } = parsePaginationParams(req);
  let { keyword, status , date, range, organizationsId } = req.query;
  try {
    const userId = req.user._id;
    const timezone = req.user.timezone;
    const { reservations, meta } = await reservationService.getavailableReservations({
