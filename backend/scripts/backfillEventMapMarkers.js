 ...
 *
 * Uses basicInfo.media.name (first image only) → basicInfo.media.marker
 *
 * Usage:
...
function parseArgs(argv) {
  ...
  for (const arg of argv) {
    if (arg === "--dry-run") opts.dryRun = true;
      ...
      opts.concurrency = Math.max(1, Number(arg.slice(14)) || 2);
  }
  return opts;
}
...
async function processEvent(ev, { dryRun, force }) {
  ...
  if (!force && media.marker && String(media.marker).trim()) {
    return { id, title, status: "skip-has-marker", marker: media.marker };
  }
  ...
  if (dryRun) {
    return {
      ...
      status: "dry-run",
      source: first,
      wouldReplace: media.marker || null,
    };
  }

  try {
    const marker = await createMapMarkerFromSource(first);
    if (!marker) {
      return { id, title, status: "fail-create", source: first };
    }

    if (media.marker && toBlobName(media.marker) !== marker) {
      await deleteBlobQuiet(media.marker);
    }

    await Events.updateOne(
      { _id: ev._id },
      { $set: { "basicInfo.media.marker": marker } }
    );
...
async function main() {
  ...
  if (!opts.force) {
    filter.$and = [
      {
        $or: [
          { "basicInfo.media.marker": { $exists: false } },
          { "basicInfo.media.marker": null },
          { "basicInfo.media.marker": "" },
        ],
      },
    ];
  }
